package ccr.app.version08;

import ccr.app.*;

import java.util.*;

public class TestCFG2_82 extends Application {
// Fault notes: 
...
//	private int mode = MODE_MIX;
	...
	//	distance = distance + Coordinates.calDist(location, lastLocation); // Experiments: calculate distance
		...
		while (stay > 0) { // 1
			...
			location = toCoordinates(candidate);
		//	distance = distance + Coordinates.calDist(location, lastLocation); // Experiments: calculate distance
			displace = Math.sqrt((location.x /*-*/+ lastLocation.x) * (location.x /*-*/+ lastLocation.x) +  // Seeded fault
					(location.y /*-*/+ lastLocation.y) * (location.y /*-*/+ lastLocation.y)); // Seeded fault
			moved = moved + toBoolean(displace);
			error = Math.sqrt((actLoc.x /*-*/+ location.x) * (actLoc.x /*-*/+ location.x) +  // Seeded fault
					(actLoc.y /*-*/+ location.y) * (actLoc.y /*-*/+ location.y)); // Seeded fault
			if (error <= ERR) {
				reliable = reliable + 1;
			}
			lastLocation = location;
