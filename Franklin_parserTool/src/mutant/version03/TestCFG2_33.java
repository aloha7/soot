...
public class TestCFG2_33 extends Application {
// Fault notes: 
...
//	private int mode = MODE_MIX;
	...
	//	distance = distance + Coordinates.calDist(location, lastLocation); // Experiments: calculate distance
		...
		while (stay > 0) { // 1
			...
			curEstX = estLoc.x;
			curEstY = estLoc.y;
			curEstX = curEstX + ((double) 2 * rand.nextDouble() - (double) /*1*/0.5) * NOISE;  // Seeded fault // [- NOISE, + NOISE)
			curEstY = curEstY + ((double) 2 * rand.nextDouble() - (double) /*1*/0.5) * NOISE;  // Seeded fault // [- NOISE, + NOISE)
			lastLoc = scenarios.getActLoc(sid, lastPos); // The actual location for lastPos
			dist = Coordinates.calDist(lastLoc, actLoc);
