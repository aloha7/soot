...
public class TestCFG2_34 extends Application {
// Fault notes: 
...
//	private int mode = MODE_MIX;
	...
	//	distance = distance + Coordinates.calDist(location, lastLocation); // Experiments: calculate distance
		...
		while (stay > 0) { // initial
			...
			// Apply the noise
			curEstX = curEstX + ((double) 2 * rand.nextDouble() - (double) 1) * /*NOISE*/(NOISE + 0.1);  // [- NOISE, + NOISE)
			curEstY = curEstY + ((double) 2 * rand.nextDouble() - (double) 1) * /*NOISE*/(NOISE + 0.1);  // [- NOISE, + NOISE)
