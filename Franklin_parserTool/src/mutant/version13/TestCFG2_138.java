package ccr.app.version13;

import ccr.app.*;

import java.util.*;

public class TestCFG2_138 extends Application {
// Fault notes: 
...
//	private int mode = MODE_MIX;
	...
	private boolean funcLocMixVeloOk(Context ctx1, Context ctx2) { // boolean function 4
		...
		// The velocity should be between vmin and vmax
		boolean result = false;
		double vmin = (/*VELOCITY*/(VELOCITY + 1.0) * ((double) t / 1000) - 2 * ERR) / ((double) t / 1000); // Seeded fault
		double vmax = (/*VELOCITY*/(VELOCITY - 1.0) * ((double) t / 1000) + 2 * ERR) / ((double) t / 1000); // Seeded fault
		double ve = dist / ((double) t / 1000);
		if (ve >= vmin && ve <= vmax) {
			result = true;
		}
