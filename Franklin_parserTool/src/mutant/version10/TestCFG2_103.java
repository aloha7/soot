package ccr.app.version10;

import ccr.app.*;

import java.util.*;

public class TestCFG2_103 extends Application {
// Fault notes: 
...
//	private int mode = MODE_MIX;
	...
	private boolean filterLocSkip1Walk(Context ctx1, Context ctx2) { // filter 4
		...
		long t1 = TimeFormat.convert((String) ctx1.get(Context.FLD_TIMESTAMP));
		long t2 = TimeFormat.convert((String) ctx2.get(Context.FLD_TIMESTAMP));
		if (c1 + 2 == c2 && t2 - t1 >= /*2*/4 * (long) (WALK_DIST / VELOCITY * 1000)) { // Seeded fault // Skipping 1 and in walk
			return true;
		} else {
			return false;
		}
