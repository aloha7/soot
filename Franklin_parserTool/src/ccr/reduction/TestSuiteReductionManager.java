...
//		}
				...
				for(int testSetIndex = testSet_startIndex; testSetIndex < testSet_endIndex; testSetIndex ++){
					...
					try {
						...
						if(str.contains("cannot get the solution")){
							...
							append(equivalentProgramElems).append("\t").append(alpha).append("\t").
							...
							size_reduction = str.substring(str.indexOf("Reduced Test Suite Size:")+ "Reduced Test Suite Size:".length(),
									str.indexOf("\nSelected Test Cases:"));
							selectedTestCases = str.substring(str.indexOf("Selected Test Cases:") + "Selected Test Cases:".length());
							
							sb.append(objectiveValue).append("\t").append(size_reduction).append("\t").append(selectedTestCases).append("\n");
						}
	...
	/**2010-11-18: compare the test set before and after reduction in terms of
	 ...
	 * @param testSet_endIndex
	 */
	public void compareTestSets(String date, String criterion, ArrayList<TestSet> testSets_beforeReduction, 
		...
		for(int sizeConstraint = beta_min; sizeConstraint < beta_max || (sizeConstraint>beta_max && 
			...
			HashMap<Double, HashMap<Integer, ArrayList<TestSet>>> alpha_beta_reducedTestSets 
				= new HashMap<Double, HashMap<Integer,ArrayList<TestSet>>>();
	...
	public static void main(String[] args) {
		...
		if( instruction.equals("createAndBuildILPModel")||instruction.equals("saveReductionResult")
				||instruction.equals("compareTestSets")){
			...
			if(args.length == 6){
				...
				beta_max = Integer.parseInt(args[5]); //26, 43, 50 for AllPolicies, All1ResolvedDU, All2ResolvedDU
				//33, 53, 60 for AllPolicies, All1ResolvedDU, All2ResolvedDU
			}
			...
			}else if(instruction.equals("compareTestingEffectiveness")){
