...
import ccr.help.OutputFormat;
import ccr.help.ResultInterpreter_QSIC10;
...
//		System.out.println(sb.toString());
	...
	public static double getPearsonCorrelationTest(double[] x, double[] y){
		...
		PearsonCorrelationEfficient = (double)sum_xx / ((double)Math.sqrt(sum_xx) * Math.sqrt(sum_yy));
...
//				sb_slope.append("\n");
	...
	public static void saveCorr_CD_MS(String date_MutationScore, boolean containHeader_MutationScore, int mutantNumber_all,
			int mutantNumber_1, int mutantNumber_2, int mutantNumber_3, String date_testpool, boolean containHeader_testpool, double alpha){
		HashMap<Integer, Double> CD_MS_all = getCorr_CD_MS(date_MutationScore, containHeader_MutationScore, 
				mutantNumber_all, date_testpool, containHeader_testpool, alpha);
		HashMap<Integer, Double> CD_MS_1 = getCorr_CD_MS(date_MutationScore, containHeader_MutationScore, 
				mutantNumber_1, date_testpool, containHeader_testpool, alpha);
		HashMap<Integer, Double> CD_MS_2 = getCorr_CD_MS(date_MutationScore, containHeader_MutationScore, 
				mutantNumber_2, date_testpool, containHeader_testpool, alpha);
		HashMap<Integer, Double> CD_MS_3 = getCorr_CD_MS(date_MutationScore, containHeader_MutationScore, 
				mutantNumber_3, date_testpool, containHeader_testpool, alpha);
		HashMap<Integer, Integer> CD_ScoreEquivalentSetSize = getCD_ScoreEquivalentSetSize(date_MutationScore,
				containHeader_MutationScore, mutantNumber_all, date_testpool, containHeader_testpool, alpha);
		
		
		Integer[] CD_ordered = (Integer[])CD_MS_all.keySet().toArray(new Integer[0]);
		Arrays.sort(CD_ordered);
		DecimalFormat formatter = new DecimalFormat("0.000000000");
		
		StringBuilder sb = new StringBuilder();
		sb.append("Size").append("\t").append("CD").append("\t").append("MS_" + mutantNumber_all).append("\t").
		append("MS_" + mutantNumber_1).append("\t").append("MS_" + mutantNumber_2).append("\t").append("MS_" + mutantNumber_3).append("\n");		
		
		for(int i = 0; i < CD_ordered.length; i ++){
			int CD = CD_ordered[i];
			int size = CD_ScoreEquivalentSetSize.get(CD);
			double MS_all = CD_MS_all.get(CD);
			double MS_1 = CD_MS_1.get(CD);
			double MS_2 = CD_MS_2.get(CD);
			double MS_3 = CD_MS_3.get(CD);
			
			sb.append(size).append("\t").append(CD).append("\t").
			append(formatter.format(MS_all)).append("\t").
			append(formatter.format(MS_1)).append("\t").
			append(formatter.format(MS_2)).append("\t").
			append(formatter.format(MS_3)).append("\n");
		}
		
		String file =  "src/ccr/experiment/Context-Intensity_backup/TestHarness/"
			+ date_testpool + "/Result/CD_MS/CD_MS_"+ mutantNumber_all + "_"+ mutantNumber_1 + "_" +
			mutantNumber_2 + "_" + mutantNumber_3 + "_" + new DecimalFormat("0.00000").format(alpha) + ".txt";
	...
	public static HashMap<Integer, Integer> getCD_ScoreEquivalentSetSize(String date_MutationScore, boolean containHeader_MutationScore, int mutantNumber
			, String date_testpool, boolean containHeader_testpool, double alpha){
		HashMap<Integer, Integer> CD_ScoreEquivalentSetSize = new HashMap<Integer, Integer>();
		...
		for(int i = 0; i < CD_ordered.length; i ++){
			int CD = CD_ordered[i];
			ArrayList tcList = CD_tcList.get(CD);
			CD_ScoreEquivalentSetSize.put(CD, tcList.size());
		}
		
		return CD_ScoreEquivalentSetSize;
	}
	
	/**2010-01-21: given a mutant number, return the averaged mutation score for each context diversity  
	 * 
	 * @param date_MutationScore
	 * @param containHeader_MutationScore
	 * @param mutantNumber
	 * @param date_testpool
	 * @param containHeader_testpool
	 * @param alpha
	 * @return: context diversity -> averaged mutation score
	 */
	public static HashMap<Integer, Double> getCorr_CD_MS(String date_MutationScore, boolean containHeader_MutationScore, int mutantNumber
			, String date_testpool, boolean containHeader_testpool, double alpha){
		HashMap<Integer, Double> CD_MS = new HashMap<Integer, Double>();
		HashMap<String, Double> tc_MS = loadMutantScore(date_MutationScore, containHeader_MutationScore, mutantNumber);
		HashMap<Integer, ArrayList> CD_tcList = getTestPool(date_testpool, containHeader_testpool, alpha);		
	...
	/**2010-01-21: for all score equivalent set in the all-level, sort
	 * them based on their mutation scores and observe the easiness of 
	 * mutations at different levels
	 * @param date
	...
	public static void main(String[] args) {
		...
		if(instruction.equals("getCorre_CD_MS_fixedLen_single")){
			...
			//the minimum test set size to report a length-equivalent-set, the default value is -1
			int lengthEquivalentSetSize_min = -1;
			if(args.length == 4){
				lengthEquivalentSetSize_min = Integer.parseInt(args[3]);
			}
			...
			String slopeFile = "src/ccr/experiment/Context-Intensity_backup/TestHarness/"
				+ date + "/Result/CD_MS_FixedLen/CI_MS_CorrelationSlope_"
				+ lengthEquivalentSetSize_min+".txt";
		...
		}else if(instruction.equals("saveCorre_CD_MS")){
			String date = args[1];
			double alpha = Double.parseDouble(args[2]);
			int mutantNumber_all = Integer.parseInt(args[3]);
			int mutantNumber_1 = Integer.parseInt(args[4]);
			int mutantNumber_2 = Integer.parseInt(args[5]);
			int mutantNumber_3 = Integer.parseInt(args[6]);
			...
			saveCorr_CD_MS(date_MutationScore, containHeader_MutationScore, mutantNumber_all,
					mutantNumber_1, mutantNumber_2, mutantNumber_3, date_testpool, containHeader_testpool, alpha);
		...
		}else if(instruction.equals("getPearsonCorrelationTest")){
			double[] a = {1, 2, 3, 4};
			double[] b = {6, 7, 8, 9};
			System.out.println(getPearsonCorrelationTest(a, b));
		}
	}
}
