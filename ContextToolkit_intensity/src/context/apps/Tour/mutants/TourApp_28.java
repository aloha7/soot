...
public class TourApp_28 implements context.arch.handler.Handler
{
    ...
    public TourApp_28( java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        this( DEFAULT_PORT, userid, configFile, demoFile );
    }

    public TourApp_28( int localport, java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        try {
            ...
            System.out.println( "TourApp_28 MalformedURL: " + mue );
        } catch ( context.arch.comm.language.DecodeException de ) {
            System.out.println( "TourApp_28 Decode: " + de );
        }
    }
                ...
                if (callback.equals( SUBSCRIBER_ID + Constants.SPACER + WTourDemo.INTEREST )) {
                    ...
                    if (callback.equals( SUBSCRIBER_ID + Constants.SPACER + WTourEnd.END )) {
                        ...
                        for (int i = 0; i < demoVisits.numDemoVisits(); i++) {
                            ...
                            message.append( dv.getDemoerUrl() + ", or go the demoer's web page at " + dv.getDemoerUrl() + ".\n\n" );
                        }
    ...
    {
        if (argv.length == 3) {
            System.out.println( "Attempting to create a TourApp_28 on " + DEFAULT_PORT + " for " + argv[0] );
            context.apps.Tour.mutants.TourApp_28 ta = new context.apps.Tour.mutants.TourApp_28( argv[0], argv[1], argv[2] );
        } else {
            if (argv.length == 4) {
                System.out.println( "Attempting to create a TourApp_28 on " + argv[1] + " for " + argv[0] );
                context.apps.Tour.mutants.TourApp_28 ta = new context.apps.Tour.mutants.TourApp_28( Integer.parseInt( argv[1] ), argv[0], argv[2], argv[3] );
            } else {
                System.out.println( "USAGE: java TourApp_28 <userid> [port] <config file> <demo info file>" );
            }
        }
    }
