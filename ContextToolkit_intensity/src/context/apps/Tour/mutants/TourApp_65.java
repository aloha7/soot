...
public class TourApp_65 implements context.arch.handler.Handler
{
    ...
    public TourApp_65( java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        this( DEFAULT_PORT, userid, configFile, demoFile );
    }

    public TourApp_65( int localport, java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        try {
            ...
            System.out.println( "TourApp_65 MalformedURL: " + mue );
        } catch ( context.arch.comm.language.DecodeException de ) {
            System.out.println( "TourApp_65 Decode: " + de );
        }
    }
        ...
        if (callback.equals( SUBSCRIBER_ID + Constants.SPACER + WTourRegistration.UPDATE )) {
            ...
            if (callback.equals( SUBSCRIBER_ID + Constants.SPACER + WTourDemo.VISIT )) {
                ...
                if (callback.equals( SUBSCRIBER_ID + Constants.SPACER + WTourDemo.INTEREST )) {
                    ...
                    java.lang.String demoName = (java.lang.String) atts.getAttributeNameValue( WTourDemo.DEMO_NAME ).getValue();
                    demoInterests.addDemoInterest(interest);
                    context.arch.storage.AttributeNameValues input = new context.arch.storage.AttributeNameValues();
                    input.addAttributeNameValue( DemoInterests.DEMO_INTERESTS, demoInterests.toAttributeNameValues(), Attribute.STRUCT );
                    input.addAttributeNameValue( WTourRegistration.INTERESTS, interests );
                    input.addAttributeNameValue( Demos.DEMOS, demo.getDemos().toAttributeNameValues(), Attribute.STRUCT );
    ...
    {
        if (argv.length == 3) {
            System.out.println( "Attempting to create a TourApp_65 on " + DEFAULT_PORT + " for " + argv[0] );
            context.apps.Tour.mutants.TourApp_65 ta = new context.apps.Tour.mutants.TourApp_65( argv[0], argv[1], argv[2] );
        } else {
            if (argv.length == 4) {
                System.out.println( "Attempting to create a TourApp_65 on " + argv[1] + " for " + argv[0] );
                context.apps.Tour.mutants.TourApp_65 ta = new context.apps.Tour.mutants.TourApp_65( Integer.parseInt( argv[1] ), argv[0], argv[2], argv[3] );
            } else {
                System.out.println( "USAGE: java TourApp_65 <userid> [port] <config file> <demo info file>" );
            }
        }
    }
