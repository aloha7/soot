...
public class TourApp_176 implements context.arch.handler.Handler
{
    ...
    public TourApp_176( java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        this( DEFAULT_PORT, userid, configFile, demoFile );
    }

    public TourApp_176( int localport, java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        try {
            ...
            System.out.println( "TourApp_176 MalformedURL: " + mue );
        } catch ( context.arch.comm.language.DecodeException de ) {
            System.out.println( "TourApp_176 Decode: " + de );
        }
    }
    ...
    {
        if (argv.length == 3) {
            System.out.println( "Attempting to create a TourApp_176 on " + DEFAULT_PORT + " for " + argv[0] );
            context.apps.Tour.mutants.TourApp_176 ta = new context.apps.Tour.mutants.TourApp_176( argv[0], argv[1], argv[2] );
        } else {
            if (argv.length == 4) {
                System.out.println( "Attempting to create a TourApp_176 on " + argv[1] + " for " + argv[0] );
                context.apps.Tour.mutants.TourApp_176 ta = new context.apps.Tour.mutants.TourApp_176( Integer.parseInt( argv[1] ), argv[0], argv[2], argv[3] );
            } else {
                System.out.println( "USAGE: java TourApp_176 <userid> [port] <config file> <demo info file>" );
            }
        }
    }
