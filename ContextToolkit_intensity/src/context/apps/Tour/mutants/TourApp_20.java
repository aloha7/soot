...
public class TourApp_20 implements context.arch.handler.Handler
{
    ...
    public TourApp_20( java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        this( DEFAULT_PORT, userid, configFile, demoFile );
    }

    public TourApp_20( int localport, java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        try {
            ...
            if (servers != null) {
                for (java.util.Enumeration e = servers.getEnumeration(); e.hasMoreElements();) {
                    ...
                    if (serverObj.getId().indexOf( userid ) != -1) {
                        ...
                        int serverPort = Integer.parseInt( serverObj.getHost() );
            ...
            System.out.println( "TourApp_20 MalformedURL: " + mue );
        } catch ( context.arch.comm.language.DecodeException de ) {
            System.out.println( "TourApp_20 Decode: " + de );
        }
    }
    ...
    {
        if (argv.length == 3) {
            System.out.println( "Attempting to create a TourApp_20 on " + DEFAULT_PORT + " for " + argv[0] );
            context.apps.Tour.mutants.TourApp_20 ta = new context.apps.Tour.mutants.TourApp_20( argv[0], argv[1], argv[2] );
        } else {
            if (argv.length == 4) {
                System.out.println( "Attempting to create a TourApp_20 on " + argv[1] + " for " + argv[0] );
                context.apps.Tour.mutants.TourApp_20 ta = new context.apps.Tour.mutants.TourApp_20( Integer.parseInt( argv[1] ), argv[0], argv[2], argv[3] );
            } else {
                System.out.println( "USAGE: java TourApp_20 <userid> [port] <config file> <demo info file>" );
            }
        }
    }
