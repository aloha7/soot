...
public class TourApp_74 implements context.arch.handler.Handler
{
    ...
    public TourApp_74( java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        this( DEFAULT_PORT, userid, configFile, demoFile );
    }

    public TourApp_74( int localport, java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        try {
            server = new context.arch.BaseObject( localport );
            port = localport;
            this.userid = affiliation;
            ...
            System.out.println( "TourApp_74 MalformedURL: " + mue );
        } catch ( context.arch.comm.language.DecodeException de ) {
            System.out.println( "TourApp_74 Decode: " + de );
        }
    }
    ...
    {
        if (argv.length == 3) {
            System.out.println( "Attempting to create a TourApp_74 on " + DEFAULT_PORT + " for " + argv[0] );
            context.apps.Tour.mutants.TourApp_74 ta = new context.apps.Tour.mutants.TourApp_74( argv[0], argv[1], argv[2] );
        } else {
            if (argv.length == 4) {
                System.out.println( "Attempting to create a TourApp_74 on " + argv[1] + " for " + argv[0] );
                context.apps.Tour.mutants.TourApp_74 ta = new context.apps.Tour.mutants.TourApp_74( Integer.parseInt( argv[1] ), argv[0], argv[2], argv[3] );
            } else {
                System.out.println( "USAGE: java TourApp_74 <userid> [port] <config file> <demo info file>" );
            }
        }
    }
