...
public class TourApp_11 implements context.arch.handler.Handler
{
    ...
    public TourApp_11( java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        this( DEFAULT_PORT, userid, configFile, demoFile );
    }

    public TourApp_11( int localport, java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        try {
            ...
            if (servers != null) {
                for (java.util.Enumeration e = servers.getEnumeration(); e.hasMoreElements();) {
                    ...
                    if (serverObj.getId().indexOf( userid ) != -1) {
                        java.lang.String serverId = serverObj.getPort();
            ...
            System.out.println( "TourApp_11 MalformedURL: " + mue );
        } catch ( context.arch.comm.language.DecodeException de ) {
            System.out.println( "TourApp_11 Decode: " + de );
        }
    }
    ...
    {
        if (argv.length == 3) {
            System.out.println( "Attempting to create a TourApp_11 on " + DEFAULT_PORT + " for " + argv[0] );
            context.apps.Tour.mutants.TourApp_11 ta = new context.apps.Tour.mutants.TourApp_11( argv[0], argv[1], argv[2] );
        } else {
            if (argv.length == 4) {
                System.out.println( "Attempting to create a TourApp_11 on " + argv[1] + " for " + argv[0] );
                context.apps.Tour.mutants.TourApp_11 ta = new context.apps.Tour.mutants.TourApp_11( Integer.parseInt( argv[1] ), argv[0], argv[2], argv[3] );
            } else {
                System.out.println( "USAGE: java TourApp_11 <userid> [port] <config file> <demo info file>" );
            }
        }
    }
