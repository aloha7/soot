...
public class TourApp_63 implements context.arch.handler.Handler
{
    ...
    public TourApp_63( java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        this( DEFAULT_PORT, userid, configFile, demoFile );
    }

    public TourApp_63( int localport, java.lang.String userid, java.lang.String configFile, java.lang.String demoFile )
    {
        try {
            ...
            if (servers != null) {
                for (java.util.Enumeration e = servers.getEnumeration(); e.hasMoreElements();) {
                    ...
                    if (serverObj.getId().indexOf( this.userid ) != -1) {
            ...
            System.out.println( "TourApp_63 MalformedURL: " + mue );
        } catch ( context.arch.comm.language.DecodeException de ) {
            System.out.println( "TourApp_63 Decode: " + de );
        }
    }
    ...
    {
        if (argv.length == 3) {
            System.out.println( "Attempting to create a TourApp_63 on " + DEFAULT_PORT + " for " + argv[0] );
            context.apps.Tour.mutants.TourApp_63 ta = new context.apps.Tour.mutants.TourApp_63( argv[0], argv[1], argv[2] );
        } else {
            if (argv.length == 4) {
                System.out.println( "Attempting to create a TourApp_63 on " + argv[1] + " for " + argv[0] );
                context.apps.Tour.mutants.TourApp_63 ta = new context.apps.Tour.mutants.TourApp_63( Integer.parseInt( argv[1] ), argv[0], argv[2], argv[3] );
            } else {
                System.out.println( "USAGE: java TourApp_63 <userid> [port] <config file> <demo info file>" );
            }
        }
    }
