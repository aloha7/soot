...
public class WTourDemo extends WPersonPresence implements AsyncServiceHandler {
   ...
   */
  public WTourDemo(String demo, String configFile, String demoFile, boolean storageFlag) {
   ...
   * @param storageFlag Flag to indicate whether storage is enabled
   */
  public WTourDemo(String demo, int port, String configFile, String demoFile, boolean storageFlag) {
    this(demo, port, configFile, demoFile, CLASSNAME+SPACER+demo, storageFlag);
  }
   ...
   * @param storageFlag Flag to indicate whether storage is enabled
   */
  public WTourDemo(String demo, int port, String configFile, String demoFile, String id, boolean storageFlag) {
  ...
  public void notify(String event, Object data) {
    ...
    if(event.equals(WTourDemo.VISIT)){
    	System.out.println(this.getClass().getName() + " gets a context:"+ WTourDemo.VISIT);
    	/*
        ...
        if (servers != null) {
          ...
          for (Enumeration e=servers.getEnumeration(); e.hasMoreElements(); ) {
          ...
          DataObject poll = pollWidget(serverHost, serverPort, serverId, attributes);
          AttributeNameValues pollAtts = new AttributeNameValues(poll);
          
          
          String display = (String)pollAtts.getAttributeNameValue(DISPLAY_DEVICE).getValue();      
         
          AttributeNameValues input = new AttributeNameValues();
          input.addAttributeNameValue(DisplayChoiceService.TITLE,"Question for demo "+demo);
          input.addAttributeNameValue(DisplayChoiceService.QUESTION,"Please indicate your interest level for "+demo);
          input.addAttributeNameValue(DisplayChoiceService.CHOICE,MIN_LABEL);
          input.addAttributeNameValue(DisplayChoiceService.CHOICE,MED_LABEL);
          input.addAttributeNameValue(DisplayChoiceService.CHOICE,MAX_LABEL);
...
//                        DisplayChoiceService.DISPLAY, input, CLASSNAME+ibutton.getId());
          ...
          DataObject service = executeAsynchronousWidgetService(this,display,8000, 
                  WDisplay.CLASSNAME+SPACER+"test", DisplayChoiceService.DISPLAY_CHOICES, 
                  DisplayChoiceService.DISPLAY, input, CLASSNAME+ibutton.getId());
...
//                          DisplayChoiceService.DISPLAY, input, "TourDemo 1");
  ...
  public DataObject asynchronousServiceHandle(String id, DataObject result) {
    ...
    AttributeNameValue att = values.getAttributeNameValue(DisplayChoiceService.CHOICE);
    if (!att.getValue().equals(DisplayChoiceFrame.NO_CHOICE)) {
      ...
      atts.addAttributeNameValue(USERID,id.substring(CLASSNAME.length()));
  ...
  public static void main(String argv[]) {
    if (argv.length == 3) {
      if (DEBUG) {
        System.out.println("Attempting to create a WTourDemo on "+DEFAULT_PORT+" at " +argv[0]+ " with storage enabled");
      }
 ...
 */
      ...
      if ((argv[3].equals("true")) || (argv[3].equals("false"))) {
        if (DEBUG) {
          System.out.println("Attempting to create a WTourDemo on "+DEFAULT_PORT+" at " +argv[0]+ " with storage set to "+argv[3]);
        }
        WTourDemo wtd = new WTourDemo(argv[0],argv[1],argv[2], Boolean.valueOf(argv[3]).booleanValue());
      }
      else {
        if (DEBUG) {
          System.out.println("Attempting to create a WTourDemo on "+argv[1]+" at " +argv[0]+ " with storage enabled");
        }
        WTourDemo wtd = new WTourDemo(argv[0],Integer.parseInt(argv[1]),argv[2],argv[3]);
      }
    }
    else if (argv.length == 5) {
      if (DEBUG) {
        System.out.println("Attempting to create a WTourDemo on "+argv[1]+" at " +argv[0]+ "with storage set to "+argv[4]);
      }
      WTourDemo wtd = new WTourDemo(argv[0],Integer.parseInt(argv[1]),argv[2],argv[3], Boolean.valueOf(argv[4]).booleanValue());
    }
    else {
      System.out.println("USAGE: java context.arch.widget.WTourDemo <demo> [port] <config file> <info file> [storageFlag]");
    }       
  }
}
