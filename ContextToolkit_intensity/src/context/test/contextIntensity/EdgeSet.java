...
public class EdgeSet implements Cloneable {
	...
	public Edge get(Node startNode){
		for(int i = 0 ; i < edges.size(); i ++){
			Edge edge = (Edge)edges.get(i);
			if(edge.sourNode.index.equals(startNode.index)){
				return edge;
			}
		}
		return null;
	...
	public NodeSet getSucc(String index) {
		NodeSet succs = new NodeSet();
		for (int i = 0; i < Succ.keySize(); i++) {
			if (((Node) Succ.getKey(i)).index.equals(index))
				succs = Succ.getValue(i);
		}
